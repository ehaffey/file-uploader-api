...
router.get('/fileUploads', requireToken, (req, res, next) => {
  FileUpload.find()
    .then(fileUpload => {
      // `fileUpload` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return fileUpload.map(fileUpload => fileUpload.toObject())
    })
    // respond with status 200 and JSON of the fileUpload
    .then(fileUpload => res.status(200).json({ fileUpload: fileUpload }))
    // if an error occurs, pass it to the handler
    .catch(next)
})

// SHOW
// GET /fileUpload/5a7db6c74d55bc51bdf39793
...
// UPDATE
// PATCH /fileUpload/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /fileUpload/5a7db6c74d55bc51bdf39793
